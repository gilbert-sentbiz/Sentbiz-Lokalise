...
module ReviewSupport
  ...
  SKILL_DIR = File.join(PROJECT_ROOT, "auto-translation")
  ...
  SECTION_ORDER = %w[existing skip new].freeze
  SECTION_TITLES = {
    "existing" => "기존 키",
    "skip" => "스킵",
    "new" => "신규 키"
  ...
  def load_secrets
    sources = [
      load_json_if_exists(LOCAL_SECRET_PATH),
      load_json_if_exists(GLOBAL_SECRET_PATH),
      ENV.to_h,
      load_json_if_exists(CLAUDE_SETTINGS_PATH)
    ]
  ...
  def request_json(url, headers)
    ...
    Net::HTTP.start(uri.host, uri.port, use_ssl: uri.scheme == "https", open_timeout: 10, read_timeout: 60) do |http|
      ...
      raise "HTTP #{res.code} for #{url}: #{res.body.to_s[0, 500]}" unless res.is_a?(Net::HTTPSuccess)
  ...
  def translate_phrase(text, term_map)
    ...
    words = english.split
    english = ([words.last] + words[0...-1]).join(" ") if words.length > 1 && words.last == "Create"
    english
  end
  ...
  def build_key_proposal(text, english)
    "#{classify_prefix(text)}#{to_pascal_case(english)}"
  end
  ...
  def render_section(title, rows)
    ...
    output << ""
    output << "```text"
    output << [fit("No", 3), fit("국문", 20), fit("영문 번역 제안", 24), fit("키 제안", 48), "상태", "검수"].join("  ")
    rows.each_with_index do |row, index|
      output << [
        fit(format("%02d", index + 1), 3),
        fit(row["ko"], 20),
        fit(row["en"].to_s.empty? ? "-" : row["en"], 24),
        fit(row["key"], 48),
        fit(row["status"], 10),
        row["decision"].to_s.empty? ? "-" : row["decision"]
      ].join("  ")
    end
    output << "```"
    output << ""
  ...
  def build_review(figma_url)
    secret = load_secrets
    figma_ref = parse_figma_url(figma_url)
    ...
    figma_api = "https://api.figma.com/v1/files/#{figma_ref.fetch('file_key')}/nodes?ids=#{URI.encode_www_form_component(figma_ref.fetch('node_id'))}"
    figma_payload, = request_json(figma_api, "X-Figma-Token" => secret.fetch("FIGMA_TOKEN"))
    ...
    review["counts"] = {
      "existing" => review["existing"].length,
      "skip" => review["skip"].length,
  ...
  def write_review_outputs(review, output_dir)
    FileUtils.mkdir_p(output_dir)
    slug = sanitize_slug(review["node_name"].to_s.empty? ? "review" : review["node_name"])
    txt_path = File.join(output_dir, "#{slug}-review.txt")
    json_path = File.join(output_dir, "#{slug}-review.json")

    sections = SECTION_ORDER.map do |section|
      render_section("#{SECTION_TITLES.fetch(section)} 1-#{review.fetch(section).length}", review.fetch(section))
    end

    File.write(txt_path, sections.join("\n"))
    File.write(json_path, JSON.pretty_generate(review) + "\n")
    [txt_path, json_path]
  end
  ...
  def summarize_review(review)
    ...
    SECTION_ORDER.each do |section|
      rows = review.fetch(section)
      lines << render_section("#{SECTION_TITLES.fetch(section)} 1-#{rows.length}", rows)
    end
    lines.join("\n")
  end
end
